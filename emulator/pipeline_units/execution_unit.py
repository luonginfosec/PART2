...
class execution_unit(object):
    ...
    def __init__(self, BIU, int_msg):
        ...
        self.opcode = ''             # opcode
        self.opd = []                # operand Operands
        self.opbyte = 2              # Default operation bytes: 1,2,4
        self.eo = [0] * 5            # Evaluated opds
        self.bus = BIU               # Internal bus connection BIU
        # Flag Register
        self.interrupt = False       # External interrupt request
        self.shutdown = False        # shutdown
        self.int_msg = int_msg       # Whether to print interrupt information
    ...
    def inc_reg(self, reg, val):
        # Increase or decrease the value of a register
        self.write_reg(reg, self.read_reg(reg) + val)

    def get_address(self, opd):
        # Resolve all memory addressing: direct, register indirect, base, indexed, base-indexed, base-relative indexed
        adr_reg = ['BX', 'SI', 'DI', 'BP']
        seg_reg = ['DS', 'CS', 'SS', 'ES']
        ...
        if not has_seg:
            if 'BP' in par_list: # When BP exists, the default segment register is SS
    ...
    def __get_char(self, address):
        # Get the ASCII character at the memory address
        return chr(to_decimal(self.bus.read_byte(address)[0]))

    def get_int(self, opd):
        # Automatically obtain the operation value
        # If opd is int, it is also accessed as an address.
    ...
    def put_int(self, opd, num):
        #Automatically store num to opd (register or memory)
        if self.is_reg(opd):
    ...
    def dos_isr_21h(self):
        ...
        al = self.read_reg('AL')
        if self.int_msg:
            self.print(f"\nCalling DOS interrupt routine 21H, AH={hex(ah)}\n")
        if ah == 0x0:
            if self.int_msg:
            ...
            if self.int_msg:
                self.print("Interrupt routine function: Keyboard input with echo\n")
            char = input()[0]
            self.write_reg('AL', ord(char)) # Store ASCII value
        ...
        elif ah == 0x9: # Display string DS:DX=string address, '$' terminates string
            if self.int_msg:
                self.print("Interrupt routine function: Display string\n")
            address = (self.read_reg('DS') << 4) + self.read_reg('DX')
            count = 0
            self.print("> " * 16)     # '>' indicates CPU output

            while True:
                char = self.__get_char(address)
                if char == '$' or count == 500: # Stop if limit reached
                    break
                self.print(char)
                address += 1
                count += 1
        ...
        elif ah == 0x2c: # Get time: CH:CL=hour:minute DH:DL=second:1/100second
            if self.int_msg:
            ...
            if self.int_msg:
                self.print("Interrupt routine function: Get interrupt vector\n")
            ...
            if self.int_msg:
                self.print("Interrupt routine function: Exit with return code\n")
    ...
    def interrupt_handler(self, int_type):
        self.inc_reg('SP', -2) # Save current state
        self.write_mem(self.ss_sp, self.FR.get_int())
        self.FR.trap = 0
        self.FR.interrupt = 0
        self.inc_reg('SP', -2)
        ...
        if self.int_msg:
            self.print(f"Executing interrupt {hex(int_type)}...\n")
            self.print("Successfully saved current state\n")
            self.print(f"Reading interrupt vector table at {hex(int_type * 4)} offset address {hex(ip_val)} => IP\n")
            self.print(f"Reading interrupt vector table at {hex(int_type * 4 + 2)} segment address {hex(cs_val)} => CS\n")
            self.print("Entering interrupt routine...\n")
    ...
    def miscellaneous_ins(self): 
        if self.opcode == 'NOP':
            ...
            if self.int_msg:
                self.print("Interrupt routine ending, restoring state...\n")
            self.opcode = 'POP'
            self.opd = ['IP']
            self.control_circuit()
            ...
            self.control_circuit()
            if self.int_msg:
                self.print("Successfully restored state\n")
